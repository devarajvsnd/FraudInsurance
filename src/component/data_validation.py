...
# Doubt exist for below code
    ...
    def is_file_exists(self)->bool:
        try:
            ...
            if not is_available:
                ...
                message=f"File: {training_file} is not present"
    ...
    def validate_dataset_schema(self)->bool:
        try:
            ...
            logging.info("Entering the validation of provided dataset")
            json_path=self.data_validation_config.schema_file_path
            dic= read_json_file(json_path)

            NumberofColumns = dic['NumberofColumns']
            column_names = dic['ColName']
            keys=list(column_names.keys())
            ...
            if train_df.shape[1] == NumberofColumns and test_df.shape[1]== NumberofColumns:
                logging.info("Train and Test file have column length {NumberofColumns}")
                validation_status = True

            logging.info("checking for name of the columns")
            if train_columns == keys and test_columns == keys:
                logging.info("Train and Test file have same column names")
                validation_status = True
